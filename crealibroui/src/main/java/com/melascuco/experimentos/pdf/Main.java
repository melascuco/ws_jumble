...
public class Main {
	...
	private void initialize() {
		...
		JLabel lblEligeUnPdf = new JLabel(
				"Elige un PDF y obtendrás una copia formateada para imprimir a dos caras por hoja a doble página, es decir, en FORMATO LIBRO");
		...
		textFieldDestino.getDocument().addDocumentListener(new DocumentListener() {
			@Override
		...
		btnCrearLibro.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (nPages % 4 != 0) {
						log.info("Se aumentará el número de páginas para que sea múltiplo de 4 porque " + nPages
								+ " no es múltiplo de 4");
